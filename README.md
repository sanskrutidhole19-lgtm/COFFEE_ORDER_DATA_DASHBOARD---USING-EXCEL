##### **<h1 align="center">Coffee Orders Analytics Dashboard 🚀</h1>**



###### **## About the Project**



The Coffee Orders Analytics Dashboard is a data analysis project developed using Microsoft Excel to visualize and monitor coffee sales data in an interactive format.



This dashboard converts raw order information into easy-to-understand visual reports that help identify trends, customer behavior, and product performance.



\---



###### **## Objective**



The main objective of this project is to:



\* Analyze coffee sales performance

\* Identify profitable products

\* Track customer purchasing patterns

\* Compare country-wise sales

\* Create a professional business dashboard



\---



###### **## Dashboard Components**



\### Sales KPIs



\* Total Revenue

\* Total Orders

\* Sales Performance Metrics



\### Analytical Sections



\* Country Sales Analysis

\* Customer Sales Analysis

\* Product-Based Insights

\* Monthly Sales Trends



\### Interactive Features



\* Slicers for filtering

\* Dynamic charts

\* Automated pivot table summaries



\---

###### 

###### **## Technologies Used**



| Tool                   | Purpose               |

| ---------------------- | --------------------- |

| Microsoft Excel        | Dashboard Development |

| Pivot Tables           | Data Aggregation      |

| Pivot Charts           | Visualization         |

| Slicers                | Interactive Filtering |

| Conditional Formatting | Highlighting Insights |



\---



###### **## Dataset Structure**



The Excel workbook contains multiple sheets:



\* Orders Data

\* Customer Data

\* Product Data

\* Country Sales

\* Customer Sales

\* Interactive Dashboard



\---



###### **## Business Insights Generated**



This dashboard helps answer questions like:



\* Which coffee products generate the highest sales?

\* Which country contributes the most revenue?

\* Who are the top customers?

\* How do sales change over time?



\---



###### **## Project Highlights**



✔ Beginner-Friendly Data Analytics Project

✔ Realistic Sales Dataset

✔ Interactive Dashboard Design

✔ Business-Oriented Visualization

✔ Strong Portfolio Project for Data Analyst Roles



\---



###### **## How to Use**



1\. Download the Excel file

2\. Open in Microsoft Excel

3\. Navigate to the `Dashboard` sheet

4\. Use slicers and filters to interact with data

5\. Explore charts and KPIs



\---



###### **## Possible Enhancements**



\* Convert dashboard into Power BI

\* Add SQL database connectivity

\* Integrate automated reporting

\* Build forecasting models



\---



###### **## Dashboard Screenshot**


<img src="https://github.com/sanskrutidhole19-lgtm/COFFEE_ORDER_DATA_DASHBOARD---USING-EXCEL/blob/main/COFFEE_ORDER_DATA_DASHBOARD_IMAGE/IMAGE.png?raw=true" width="1000">





\---



###### **## Project Purpose**



This project was created for:



\* Data Analytics Practice

\* Portfolio Building

\* Excel Dashboard Learning

\* Business Data Visualization Experience



\---



###### **## Author**



Sanskruti Dhole



Data Analytics Enthusiast | Excel Dashboard Developer



